...
const UserAdmin = ({ users, success, reload, userTitle }) => {
    ...
    return (
        <Fragment>
            <Layout>
                <Head title={general.title + ' - ' + userTitle} />
                    ...
                    <div className=" flex items-center justify-between py-4 border-b border-gray-300">
                        <div className="relative">
                            <InertiaLink
                                as="button"
                                type="button"
                                href={route('usuario.create')} className="flex items-center mb-2 md:mb-0 bg-blue-500 hover:bg-blue-700 px-5 py-2 text-sm shadow-sm border-2 border-white text-gray-100 rounded-lg hover:shadow-lg">
                            ...
                            <tbody>
                                {users.data.map((user, index) => (
                                    <tr key={index} className="border-b hover:bg-orange-100 bg-gray-100">
                                        <td className="p-1">{user.id_user}</td>
                                        <td className="p-1">{user.name}</td>
                                        <td className="p-1">{user.email}</td>
                                        <td className="p-2">{user.rule == 1 ? "Administrador" : "Usuário" }</td>
                                        <td className="p-1">{dataFormatada(user.created_at)}</td>
                                        <td className="p-1">{user.active == 1 ? <HiOutlineCheck className="text-2xl text-green-600" /> : <HiX className="text-2xl text-red-600" />}</td>
                                        <td className="p-1 flex justify-end">
                                            ...
                                            <button
                                                ...
                                                // type="button"
                                                // href={"#"}
                                                //href={route('pagina.destroy', user.id_post)}
